...
The result is 23. Always round sample size estimates up. We would take 23 as the minimum number of coin flips we need to achieve 80% power. 
...
What is the power of our test if we flip the coin 50 times and lower our Type I error tolerance to 0.01? Notice we leave out the `power` argument, add `n = 50`, and change `sig.level = 0.01`:
...
The power of our test is greater than 90%. 

We specifed `alternative = "greater"` since we assumed the coin was loaded for more heads (not less). This is a stronger assumption than assuming that the coin is simply unfair in one way or another. In practice, sample size and power calculations will usually make this more conservative "two-sided" assumption. In fact this is the default for pwr functions with an `alternative` argument. If we wish to assume a "two-sided" alternative, we can simply leave it out of the function. Notice the increase in sample size from 23 to 29 if we assume a "two-sided" alternative:
...
As we demonstrated with the `plot` function above, we can save our results. This produces a list object from which we can extract quantities for futher manipulation. For example, we can calculate power for sample sizes ranging from 10 to 100 in steps of 10, with an assumed "medium" effect of 0.5, and output to a data frame with some formatting:
...
proportion, we don't know which.) We would like to detect a difference as small as
...
If we think one group proportion is 15% and the other 10%:
...
Notice the difference in sample sizes. Also notice that sample size is _per group_. 
...
and calculate the mean purchase price for each gender. How powerful is this experiment if I want
...
cents in the mean purchase price. We convert that to an effect size using the following formula:

$$d = \frac{m_{1} - m_{2}}{\sigma} $$

where $m_{1}$ and $m_{2}$ are the means of each group, respectively, and $\sigma$ is the common standard deviation of the two groups.
...
maximum and minimum values and divide by 4 (or 6). Let's say the maximum purchase is $10 and the minimum purchase is $1. Our estimated standard deviation is (10 - 1)/4 = 2.25. Therefore our effect size is 0.75/2.25 $\approx$ 0.333.
...
Performing the same analsis with the base R function `power.t.test` is a little easier. The difference $m_{1} - m_{2} =$ 0.75 is entered in the `delta` argument and the estimated $\sigma$ = 2.25 is entered in the `sd` argument:
...
or with `power.t.test`, setting `type = "one.sample"` and `alternative = "one.sided"`:
...
"Paired" t-tests are basically the same as one-sample t-tests, except our one sample is usually a differences in pairs. The following example should make this clear.
...
Not all that powerful. How many high school boys should we sample for 80% power?
...
To calculate power, specify effect size (`w`), sample size (`N`), and degrees of freedom (`df` = 4 - 1).
...
variables. I set my significance level to 0.01. To determine effect 
...
We might also consider reframing the question as a two-sample proportion test. What sample 
...
Notice this returns the same sample size as the previous calculation with `pwr.chisq.test`, except divided by 2 since it's _per group_.


### pwr.r.test - correlation test

A web developer wants to conduct an experiment with one of her sites. 
She wants to randomly select a group of people, ranging in 
age from 18 - 65, and time them how long it takes them to complete a task, say
locate some piece of information. She suspects there may be a "small" positive 
linear relationship between time it takes to complete the task and age. How 
many subjects does she need to detect this positive (ie, _r_ > 0) relationship with
...
```{r}
pwr.r.test(r = 0.1, sig.level = 0.01, power = 0.8, alternative = "two.sided")

```

There is nothing tricky about the effect size argument, `r`, for this test. It is simply the hypothesized correlation.


### pwr.anova.test - balanced one-way analysis of variance tests

The driver of a diesel-powered car decides to test the quality of three types of fuel sold in his area
...
We can also use the `power.anova.test` function that comes with base R. It requires between-group and within-group _variances_. To get the same result as `pwr.anova.test` we need to square the standard deviations to get variances and multiply the between-group variance by $\frac{k}{k-1}$. This is because the effect size formula for the ANOVA test assumes the between-group variance has a demoninator of _k_ instead of _k - 1_.
...
A director of admissions at a universty wants to determine how accurately students' grade-point 
averages (gpa) at the end of their first year can be predicted or explained by SAT scores and high school class rank.
A common approach to answering this kind of question is to model gpa as a function of SAT score and class rank. Or to put another way, performing a multiple regression with gpa as the dependent variable and SAT and class rank as independent variables. 

The null hypothesis is that none of the independent variables explain any of the variability in gpa. This would mean their regression coefficients are statistically indistinguishable from 0. The alternative is that at least one of the coefficients is not 0. This is tested with an F test. 
...
What is the power of our test with 40 subjects and a significance level of 0.01? Recall $v = n - u - 1$.

```{r}
pwr.f2.test(u = 2, v = 42 - 2 - 1, f2 = 0.3/(1 - 0.3), sig.level = 0.01)
