...
Even though the absolute difference between proportions is the same (5%), the optimum sample size is now 681 per group. 15% vs 10% is actually a bigger difference than 55% vs 50%. A heuristic approach for understanding why is to take the ratios: 55/50 = 1.1 while 15/10 = 1.5. 
...
addSegs(p1 = 0.5, p2 = 0.55) # 50% vs 55%
addSegs(p1 = 0.05, p2 = 0.1) # 5% vs 10%
```
...
and calculate the mean purchase price for each gender. We'll test for a difference in means using a two-sample t-tes. How powerful is this experiment if we want
...
(_From Hogg & Tanis, exercise 6.5-12_) 24 high school boys are put on a ultraheavy rope-jumping program. 
...
If our estimated effect size is correct, we only have about a 67% chance of finding it (ie, rejecting the null hypothesis of equal preference).
...
About 744 per group. Notice that 744 $\times$ 2 = 1,488, the sample size returned perviously by `pwr.chisq.test`. In fact the test statistic for a two-sample proportion test and chi-square test of association are one and the same.
...
linear relationship between these two quantities. She will measure this relationship with correlation, _r_, and conduct a correlation test to determine if the estimated correlation is statisically greater than 0.
How many subjects does she need to sample to detect this small positive (ie, _r_ > 0) relationship with
...
(_From Kutner, et al, exercise 8.43_) A director of admissions at a universty wants to determine how accurately students' grade-point 
